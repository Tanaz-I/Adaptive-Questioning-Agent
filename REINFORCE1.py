...
class AdaptiveAgent:
    ...
    def __init__(self, topics_difficulty, prerequisites,
        ...
        self.online_log_probs  = []
        self.online_rewards    = []
        self.online_entropies  = []
        self.session_step      = 0
    ...
    # [ONLINE] called to get next question for real/simulated student
    def select_action_online(self, state_vector):
        action_idx, log_prob, entropy = self.select_action(state_vector, training=True)

        # store log_prob and entropy — reward stored after student answers
        self.online_log_probs.append(log_prob)
        self.online_entropies.append(entropy)

        return action_idx


    # [ONLINE] called after student answers
    def record_student_response(self, topic, score, difficulty, question_type):
        # guard against unpaired call
        if len(self.online_log_probs) == len(self.online_rewards):
            return None
        ...
        # trigger REINFORCE update every N questions
        if self.session_step % self.online_update_every == 0:
            self._online_reinforce_update()

        return reward


    # [ONLINE] REINFORCE update on current chunk
    def _online_reinforce_update(self):
        if len(self.online_rewards) < 2:
            return

        self.update_policy_pretrain(
            self.online_log_probs,
            self.online_rewards,
            self.online_entropies,
            gamma=0.99,
            entropy_coef=0.05   # lighter entropy for online — less exploration needed
        )
    ...
    # [ONLINE] flush remaining transitions at session end
    def end_session(self):
        # drop unpaired log_prob if student quit without answering last question
        if len(self.online_log_probs) > len(self.online_rewards):
            self.online_log_probs  = self.online_log_probs[:len(self.online_rewards)]
            self.online_entropies  = self.online_entropies[:len(self.online_rewards)]
