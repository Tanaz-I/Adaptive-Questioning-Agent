...
class PPOAgent:
    ...
    def select_action(self, state_vector, hidden=None, training=False):
        ...
        if self.use_lstm:
            ...
            logits, value, new_hidden = self.ac_network(
                state,
                hidden
            )
    ...
    def run_episode(self, n_questions=25):
        ...
        for step in range(n_questions):
            ...
            # CHANGED detach frequency
            if hidden is not None and step % 8 == 0:
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, n, self.mini_batch):
                ...
                if self.use_lstm:
                    ...
                    logits, values, _ = self.ac_network(
                        s_batch.unsqueeze(1)
                    )
    ...
    def pretrain(self, n_episodes=2000, n_questions=500):
        ...
        while episode < n_episodes:
            ...
            progress = max(
                0.0,
                (episode / n_episodes - warmup_frac) / decay_frac
            )
            ...
            self.entropy_coef = max(
                min_entropy,
                base_entropy * (1.0 - progress)
            )

            self.clip_eps = max(
                0.15,
                0.20 - 0.05 * (episode / n_episodes)
            )
            ...
            for _ in range(self.rollout_episodes):
                ...
                # CHANGED advantage normalization location
                adv = (
                    adv - adv.mean()
                ) / (adv.std() + 1e-8)
        ...
        plt.title(
            f'Smoothed PPO Training Loss '
            f'({"LSTM" if self.use_lstm else "MLP"})'
        )
